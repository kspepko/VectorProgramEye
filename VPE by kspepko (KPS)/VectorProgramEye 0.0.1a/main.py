...
def main(page: ft.Page):
    ...
    def kill_browsers(event):
        browsers = ["brave.exe", "chrome.exe", "duckduckgo.exe", "msedge.exe", "firefox.exe", "tor.exe", "yandex.exe"]
    ...
    def monitor_events():
        while True:
            if notifications["mic"] and check_mic_usage():
                subprocess.Popen(["python", "notify_mic.py"])
            if notifications["camera"] and check_camera_status():
                subprocess.Popen(["python", "notify_camera.py"])
            if notifications["screen_share"] and is_screen_sharing_active():
                subprocess.Popen(["python", "notify_screen_share.py"])
            if notifications["external_ip"] and check_external_ip_usage():
                subprocess.Popen(["python", "notify_external_ip.py"])
            if notifications["device_connect"] and check_new_device_connected():
                subprocess.Popen(["python", "notify_device_connect.py"])
            time.sleep(5)
    ...
    def check_camera_status():
        # Логика проверки статуса камеры
        # Реализуйте проверку состояния камеры
        return False  # Верните True, если камера включена

    def is_screen_sharing_active():
        def enumHandler(hwnd, lParam):
            if win32gui.IsWindowVisible(hwnd):
                title = win32gui.GetWindowText(hwnd)
                if "Discord" in title or "Zoom" in title:
                    print(f"Обнаружено окно с заголовком: {title}")
                    return True
